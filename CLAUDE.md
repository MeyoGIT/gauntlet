...
**Historique** — bouton "Historique" dans `ChallengeMode`, toujours visible à côté de "Stats" (affiche un état vide via `RunHistory` s'il n'y a encore aucune run terminée, plutôt que de disparaître).

**Pause du chrono** — colonne `gauntlet_sessions.paused_at` (`NULL` = pas en pause, sinon horodatage du début de pause). `useGauntlet.togglePause` : à la mise en pause, pose juste `paused_at` ; à la reprise, décale tous les timestamps en cours (`challenge_started_at`, `current_run_started_at`, `current_game_started_at`) du temps écoulé en pause puis remet `paused_at` à `null`. Ce décalage évite d'apprendre la notion de pause à tout le reste du calcul de durée (site, `/obs`, `game_attempts`, `run_history`) — ils continuent de faire `now - started_at` sans rien savoir des pauses. `useTimer` accepte un second paramètre `pausedAt` qui fige l'affichage au lieu de le faire avancer. Pendant la pause, `nextGame`/`failRun` sont no-op côté hook (double protection avec les boutons désactivés côté UI) pour ne jamais enregistrer une durée faussée. Bouton Pause/Reprendre admin-only dans le footer de `ChallengeMode`, avec indicateur visuel sur `/` (badge + overlay sur la grille) et sur `/obs` (icône + bordure jaune, timer figé).
