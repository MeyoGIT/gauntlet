    ...
    <div style={{ background: 'transparent' }} className="p-3 inline-block">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="inline-flex items-center gap-4 bg-black/80 backdrop-blur-sm rounded-2xl px-4 py-3"
          style={{ width: 380 }}
        >
          {isCompleted ? (
            <div className="flex items-center gap-4 w-full">
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  <span className="text-xs text-white/50 tabular-nums">{formatDuration(elapsed)}</span>
                </motion.div>
              </div>
            </div>
          ) : currentGame ? (
            <>
              {currentGame.cover_url ? (
                ...
                <div className="rounded-lg bg-white/10 shrink-0" style={{ width: 88, height: 62 }} />
              )}

              <div className="flex-1 min-w-0 flex flex-col gap-1">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-[16px] font-bold text-white leading-snug line-clamp-1 flex-1 min-w-0">
                    {currentGame.name}
                  </p>
                  <div className="flex items-center gap-1 text-white/50 shrink-0 mt-0.5">
                    ...
                    <span className="text-xs tabular-nums font-medium">{formatDuration(elapsed)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1 text-white/50 ml-auto shrink-0">
                    ...
                    <span className="text-xs tabular-nums font-medium">Run n°{session.current_run_number}</span>
                  </div>
                </div>
              </div>
            </>
